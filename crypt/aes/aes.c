...
void ECB(int arr[])
{
    ...
    printf("ECB加密的密文为\n");
    for(t1=0;t1<dataLen;t1++) //输出密文
    {
        //if(t1!=0 && t1%4==0)
            //printf("\n");
        printf("%d\t", ciphertext[t1]);
    }
    printf("\n");
    printf("---------------------------------------------\n");
}
...
void CFB(int arr[])
{
    ...
    int a[8][2];
    int dataCount = 0;  //位置变量
    int c, t, i, vk, k2, j, t1;
    for(c=0;c<8;c++)
    {
        for(t=0;t<2;t++)
        {
            a[c][t] = data[dataCount];
            dataCount++;
        }
    }
    dataCount = 0;  //恢复初始化设置
    ...
    for(i=0;i<2 * encLen;i++) //外层加密循环
    {
        //产生K
        for(vk=0;vk<encLen;vk++)
        {
            k[vk] = lv[vk];
        }
        encode(k);
...
void OFB(int arr[])
{
    ...
    int a[8][2];
    int dataCount = 0;  //位置变量
    int kk, t, i, vk, k2, j, t1;
    for(kk=0;kk<8;kk++)
    {
        for(t=0;t<2;t++)
        {
            a[kk][t] = data[dataCount];
            dataCount++;
        }
    }
    dataCount = 0;  //恢复初始化设置
